...
export function ProjectDetailsPage() {
  ...
  const startablePackages = project.workPackages.filter(
    (workPackage) => workPackage.status === "TODO"
      && workPackage.incomingDependencies.every((dependency) =>
        project.workPackages.find((item) => item.id === dependency.predecessorId)?.status
          === "COMPLETED"),
  );
  ...
  return <section>
    ...
    <div className="portfolio-section-heading actuals-heading"><div><h3>Completed work logs</h3><p>Only confirmed logs reduce remaining scope and become actual project cost. Planned allocations do not.</p></div><button className="primary-button" disabled={inProgressPackages.length === 0} type="button" onClick={() => setShowLogForm((value) => !value)}>+ Record completed work</button></div>
    {inProgressPackages.length === 0 && <div className="panel actuals-start-guide"><div><strong>Start a Work Package before recording completed work.</strong><span>A work log must belong to active scope; this prevents planned hours from being reported as completed by mistake.</span></div>{startablePackages.length > 0 ? <div className="actuals-start-actions">{startablePackages.map((workPackage) => <button className="secondary-button" disabled={isBusy} key={workPackage.id} type="button" onClick={() => void updatePackage(workPackage, { status: "IN_PROGRESS" })}>Start {workPackage.name}</button>)}</div> : <small>No unblocked Work Packages can be started yet. Complete their dependencies first.</small>}</div>}
